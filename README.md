...
### Execution strategies — the two Dispatcher adapters

The `Dispatcher` port has two interchangeable adapters. Switching between them is a one-line change in the composition root; nothing else moves.
...
Other execution strategies — Combine schedulers, callback-based SDKs, custom thread pools — are of course possible, but each has a *native shape* (a publisher, a completion handler, a delegate) that doesn't fit a unified `async throws -> T` port without coercion. Coercing them produces redundant code: a Combine "dispatcher" that wraps a `Future` and immediately collapses it via continuation is mechanically identical to `GCDDispatcher`, just with extra ceremony, and it destroys the composition that makes Combine valuable in the first place. If you need one of those mechanisms, wire it directly at the composition root in its native form rather than forcing it through this port.
...
- `Dispatcher` is a port. `SwiftConcurrencyDispatcher` and `GCDDispatcher` are two adapters for the same port. They demonstrate the meaningful architectural difference — running blocking work on Swift's cooperative pool versus on GCD's separate pool — without the domain noticing.
